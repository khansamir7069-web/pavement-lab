...
## 8. Phase 10 close-out — completed 2026-05-15

**Phase 10 status: COMPLETE.** This section is the closing block of Phase 10 (Pavement Condition Survey + Distress Assessment), not a separate phase. It bundles the F3 / F5 / F6 deferred audit items together with the PCI-calibration centralization, banner-styling consolidation and Phase-11/12 reserved-field editor hooks so the module ships as a single coherent deliverable.

| Fix | What changed | Files |
|---|---|---|
| **F3** compaction-blows visibility | The mix-code header on the gradation tab now appends `· Compaction: N blows/face` from `MIX_SPECS[k].compaction_blows_each_face`. The Word report's Project Information table carries a new "Compaction (each face)" row citing the spec value (with source applicable_code). | `app/ui/widgets/inputs_panel.py`, `app/reports/word_report.py` |
| **F5** spec-driven target air voids | New `spec_target_air_voids(mix_spec)` helper computes the midpoint of `air_voids_min_pct` and `air_voids_max_pct`. `compute_mix_design` and `parse_summary_excel` both pass the per-mix midpoint into `properties_at_obc(...)`. The Word report's OBC line cites the spec range as the source of the target value. Backwards-compatible: callers passing no spec get the legacy 4.0 fallback. DBM-II / BC / BM midpoints are all 4.0 so parity tests stay 16/16. | `app/core/obc.py`, `app/core/models.py`, `app/core/import_summary.py`, `app/reports/word_report.py` |
| **F6** project-form placeholder suffix | The mix-type combo now annotates non-IRC-verified mixes with a `[placeholder]` suffix in the visible label, computed from `MIX_TYPES[k].status`. The combo's stored data (the bare mix code) is unchanged, so existing saved projects round-trip via `findData()`. | `app/ui/widgets/project_form.py` |
| **PCI calibration centralized** | New `PCICalibration` dataclass collects every numeric constant (severity weights, distress weights, extent divisors) in one place. `get_pci_calibration()` / `set_pci_calibration()` / `reset_pci_calibration()` let a future phase swap calibrations without touching engine, panel or report code. `SEVERITY_WEIGHTS` / `DISTRESS_TYPES` are now in-place views over the active calibration so existing imports continue to track changes. | `app/core/condition_survey/distress_types.py`, `app/core/condition_survey/engine.py`, `app/core/condition_survey/__init__.py`, `app/core/__init__.py` |
| **Banner-styling consolidation** | New `PlaceholderBanner`, `InfoBanner`, `FutureExpansionBanner` widgets in `common.py` and new `add_placeholder_banner(doc, text)` helper in `_docx_common.py` replace four duplicated inline QSS strings across `inputs_panel.py`, `results_panel.py`, `condition_survey_panel.py`, `traffic_panel.py` and the orange-paragraph idiom in `word_report.py` and `condition_report.py`. Visual output is byte-identical; future styling changes touch one place. | `app/ui/widgets/common.py`, `app/reports/_docx_common.py`, + call-site swaps in 5 files |
| **Reserved-field editor hooks** | Condition-survey panel grows a new card with three STUB editors bound to `ConditionSurveyInput.image_paths`, `.ai_classification_hint`, `.gis_geometry_geojson`. Values round-trip via existing JSON persistence; the engine still ignores them. Phase 11/12 can swap stubs for working dialogs without touching the dataclass or DB schema. | `app/ui/widgets/condition_survey_panel.py` |

**Smoke coverage:** `python -m tests._smoke_phase10_condition` extended with five new sections — centralized-calibration swap (default → doubled severities → reset), F5 spec-midpoint assertion (`DBM-II` midpoint = 4.0; `None` fallback = 4.0), F3 Word-report contains "Compaction (each face)" + "75 blows" + spec-midpoint annotation, F6 mix-type combo annotates exactly the placeholder mixes (SMA placeholder, DBM-II / BC-II verified), reserved-field round-trip through save + `set_project()` reload. All prior smokes (parity 16/16, Phase 5/6/7/8/9, smoke_ui, smoke_export) remain green.

**Deferred / out of scope:** No AI/image classification (Phase 11+), no GIS parsing (Phase 12+), no EXE build, no DB migration, no protected-engine refactor.

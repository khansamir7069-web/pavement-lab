"""Pure-Python calculation engine — Excel-exact Marshall mix design."""

from .interpolation import linear_interpolate, bracket_interpolate
from .gradation import GradationInput, GradationResult, compute_gradation
from .specific_gravity import (
    CoarseAggSGInput,
    FineAggSGInput,
    BitumenSGInput,
    SpecificGravityResult,
    compute_coarse_sg,
    compute_fine_sg,
    compute_bitumen_sg,
    compute_bulk_sg_blend,
)
from .gmb import GmbSpecimen, GmbGroup, GmbInput, GmbResult, compute_gmb
from .gmm import GmmSampleRaw, GmmInput, GmmResult, compute_gmm
from .stability_flow import (
    StabilitySpecimen,
    StabilityFlowInput,
    StabilityFlowResult,
    compute_stability_flow,
)
from .marshall import MarshallSummary, MarshallRow, build_marshall_summary
from .obc import compute_obc, properties_at_obc, OBCResult, spec_target_air_voids
from .compliance import (
    MixSpec,
    MixTypeRecord,
    ComplianceResult,
    check_compliance,
    reload_specs,
    save_specs,
    MIX_SPECS,
    MIX_TYPES,
)
from .models import MixDesignInput, MixDesignResult, compute_mix_design
from .material_calc import (
    MaterialCalcInput,
    MaterialBlock,
    DryMaterialRow,
    MaterialCalcResult,
    compute_material_calc,
)
from .import_summary import ImportedMixResult, parse_summary_excel
from .binders import BinderGrade, BINDER_GRADES, PROPERTY_LABELS, load_binders, reload_binders
from .code_refs import CodeRef, CodeRecord, CODE_REGISTRY, get_code, parse_refs, labels as code_labels
from .config_profiles import (
    APPLICATION_CONFIG_ENV_VAR,
    CONFIG_SCHEMA_VERSION,
    PROFILE_CONSULTANCY,
    PROFILE_DEFAULT,
    PROFILE_DEMO,
    PROFILE_LAB,
    SUPPORTED_PROFILE_KEYS,
    ApplicationConfig,
    ApplicationProfile,
    ConfigMetadata,
    ConfigValidationIssue,
    ConfigValidationResult,
    config_report_rows,
    list_application_profiles,
    load_application_config,
    normalize_profile_key,
    resolve_application_config,
    validate_application_config,
    validate_config_payload,
    validate_profile_key,
)
from .deployment import (
    DEPLOYMENT_CHECK_FAIL,
    DEPLOYMENT_CHECK_PASS,
    DEPLOYMENT_CHECK_WARN,
    DEPLOYMENT_MANIFEST_FORMAT,
    DEPLOYMENT_MANIFEST_VERSION,
    DEPLOYMENT_SEVERITY_ERROR,
    DEPLOYMENT_SEVERITY_INFO,
    DEPLOYMENT_SEVERITY_WARNING,
    LOCAL_INSTALLER_PREPARATION_FORMAT,
    LOCAL_INSTALLER_PREPARATION_VERSION,
    DeploymentChecklistItem,
    DeploymentDiagnostics,
    DeploymentIssue,
    DeploymentManifest,
    DeploymentPackagingChecklist,
    LocalInstallerAssetCheck,
    LocalInstallerPreparationChecklist,
    RuntimePathCheck,
    build_deployment_manifest,
    build_deployment_packaging_checklist,
    build_local_installer_preparation_checklist,
    default_runtime_paths,
    format_deployment_checklist_item_text,
    format_deployment_packaging_checklist_text,
    startup_deployment_diagnostics,
    validate_runtime_environment,
    validate_runtime_path,
    write_deployment_manifest,
)
from .benchmark_datasets import (
    BENCHMARK_DATASET_READINESS_FORMAT,
    BENCHMARK_DATASET_READINESS_VERSION,
    BenchmarkDatasetCheck,
    BenchmarkDatasetReadinessChecklist,
    build_benchmark_dataset_readiness_checklist,
)
from .material_quantity import (
    LayerInput,
    LayerResult,
    MaterialQuantityInput,
    MaterialQuantityResult,
    LAYER_TYPES,
    compute_layer,
    compute_material_quantity,
)
from .traffic import (
    TrafficInput,
    TrafficResult,
    TERRAINS,
    LANE_CONFIGS,
    vdf_preset,
    ldf_preset,
    traffic_category,
    compute_traffic_analysis,
)
from .structural_design import (
    StructuralInput,
    PavementLayer,
    StructuralResult,
    compute_design_traffic,
    compute_subgrade_mr,
    suggest_composition,
    compute_structural_design,
)
from .maintenance import (
    OverlayInput,
    OverlayResult,
    compute_overlay,
    characteristic_deflection,
    allowable_deflection,
    temperature_corrected_deflection,
    ColdMixInput,
    ColdMixResult,
    compute_cold_mix,
    MicroSurfacingInput,
    MicroSurfacingResult,
    compute_micro_surfacing,
)
from .condition_survey import (
    DISTRESS_TYPES,
    SEVERITY_LEVELS,
    DistressType,
    DistressRecord,
    ConditionSurveyInput,
    ConditionSurveyResult,
    PerDistressBreakdown,
    RehabRecommendation,
    PCICalibration,
    DEFAULT_CALIBRATION as PCI_DEFAULT_CALIBRATION,
    get_calibration as get_pci_calibration,
    set_calibration as set_pci_calibration,
    reset_calibration as reset_pci_calibration,
    compute_condition_survey,
    condition_category,
    recommend_rehab,
    RECALIBRATE_ME as PCI_RECALIBRATE_ME,
    PLACEHOLDER_NOTE as PCI_PLACEHOLDER_NOTE,
)
from .mechanistic_validation import (
    DEFAULT_FATIGUE_CALIBRATION,
    DEFAULT_RUTTING_CALIBRATION,
    FatigueCalibration,
    FatigueCheck,
    LABEL_FATIGUE,
    LABEL_RUTTING,
    MechanisticValidationInput,
    MechanisticValidationSummary,
    PLACEHOLDER_NOTE as MECH_VALIDATION_PLACEHOLDER_NOTE,
    REFERENCES as MECH_VALIDATION_REFERENCES,
    REFUSED_MISSING_E_BC,
    REFUSED_MISSING_STRAIN,
    REFUSED_PLACEHOLDER_MECH,
    RuttingCalibration,
    RuttingCheck,
    StrainExtraction,
    compute_fatigue_life,
    compute_mechanistic_validation,
    compute_rutting_life,
    extract_fatigue_strain,
    extract_rutting_strain,
    get_fatigue_calibration,
    get_rutting_calibration,
    refused_fatigue_check,
    refused_rutting_check,
    reset_fatigue_calibration,
    reset_rutting_calibration,
    set_fatigue_calibration,
    set_rutting_calibration,
)
from .iitpave import (
    DEFAULT_EXE_FILENAME_POSIX as IITPAVE_DEFAULT_EXE_FILENAME_POSIX,
    DEFAULT_EXE_FILENAME_WIN as IITPAVE_DEFAULT_EXE_FILENAME_WIN,
    IITPAVE_EXECUTABLE_ENV_VAR,
    IITPAVE_RUNNER_EXTERNAL,
    IITPAVE_RUNNER_STUB,
    INPUT_FORMAT_VERSION as IITPAVE_INPUT_FORMAT_VERSION,
    STUB_OUTPUT_VERSION as IITPAVE_STUB_OUTPUT_VERSION,
    SUPPORTED_IITPAVE_RUNNER_MODES,
    EvaluationPoint as IITPaveEvaluationPoint,
    ExternalExeRunner as IITPaveExternalExeRunner,
    IITPaveRunner,
    IITPaveRunnerConfig,
    IITPaveRunnerSelectionResult,
    IITPaveEnvironmentIssue,
    IITPaveEnvironmentValidationResult,
    IITPaveExecutableCandidate,
    IITPaveDryRunConfig,
    IITPaveDryRunResult,
    IITPaveFixtureIntakeResult,
    IITPaveFixtureRecord,
    IITPaveOutputContractResult,
    IITPaveParserContractMarker,
    IITPaveParserContractSection,
    IITPaveParserContractHarnessResult,
    IITPaveVerifiedParserContractResult,
    IITPaveSchemaSectionMapping,
    IITPaveVerifiedFixtureSchemaMappingResult,
    IITPaveFixtureSchemaManifest,
    IITPaveFixtureSchemaManifestEntry,
    IITPaveSchemaFamilyCoverage,
    LoadConfig as IITPaveLoadConfig,
    MechanisticResult,
    PLACEHOLDER_NOTE as IITPAVE_PLACEHOLDER_NOTE,
    PavementLayer as IITPavePavementLayer,
    PavementStructure,
    PointResult as IITPavePointResult,
    REFERENCES as IITPAVE_REFERENCES,
    SOURCE_EXTERNAL as IITPAVE_SOURCE_EXTERNAL,
    SOURCE_STUB as IITPAVE_SOURCE_STUB,
    StubRunner as IITPaveStubRunner,
    build_iitpave_input,
    build_iitpave_fixture_schema_manifest,
    bundled_iitpave_dir,
    bundled_iitpave_exe_candidates,
    bundled_iitpave_exe_path,
    check_iitpave_dry_run_readiness,
    intake_iitpave_fixture_folder,
    intake_iitpave_output_fixture,
    default_evaluation_points,
    discover_iitpave_executable,
    from_structural_layers as iitpave_from_structural_layers,
    iitpave_runner_config_from_mapping,
    inspect_iitpave_output_contract,
    inspect_iitpave_verified_parser_contract,
    is_known_stub_output,
    map_iitpave_verified_fixture_schema,
    parse_iitpave_output,
    run_iitpave_parser_contract_harness,
    select_iitpave_runner,
    validate_iitpave_environment,
    DEFAULT_FIXTURE_MANIFEST,
    DEFAULT_OUTPUT_SUFFIXES,
    FIXTURE_STATUS_OPERATOR_PENDING,
    FIXTURE_STATUS_REJECTED,
    FIXTURE_STATUS_UNVERIFIED,
    FIXTURE_STATUS_VERIFIED,
    SUPPORTED_FIXTURE_STATUSES,
    IITPAVE_OUTPUT_FORMAT_REAL_PENDING,
    IITPAVE_OUTPUT_FORMAT_STUB,
    IITPAVE_OUTPUT_FORMAT_UNKNOWN,
    IITPAVE_OUTPUT_STATUS_EMPTY,
    IITPAVE_OUTPUT_STATUS_MISSING,
    IITPAVE_OUTPUT_STATUS_REAL_CONTRACT_PENDING,
    IITPAVE_OUTPUT_STATUS_STUB_CONTRACT,
    IITPAVE_OUTPUT_STATUS_UNREADABLE,
    IITPAVE_OUTPUT_STATUS_UNSUPPORTED,
    IITPAVE_PARSER_CONTRACT_STATUS_BLOCKED,
    IITPAVE_PARSER_CONTRACT_STATUS_INCOMPLETE,
    IITPAVE_PARSER_CONTRACT_STATUS_PARTIAL,
    IITPAVE_PARSER_CONTRACT_STATUS_SECTIONS_DETECTED,
    IITPAVE_PARSER_CONTRACT_STATUS_UNSUPPORTED,
    IITPAVE_SECTION_HEADER,
    IITPAVE_SECTION_LABELED_BLOCK,
    IITPAVE_SECTION_TABLE_LIKE,
    IITPAVE_SCHEMA_FAMILY_DIRECT_STRESS_STRAIN_TABLE,
    IITPAVE_SCHEMA_FAMILY_ELASTIC_LAYER_STRESS_STRAIN_TABLE,
    IITPAVE_SCHEMA_FAMILY_UNKNOWN,
    IITPAVE_SCHEMA_MAPPING_STATUS_BLOCKED,
    IITPAVE_SCHEMA_MAPPING_STATUS_MAPPED,
    IITPAVE_SCHEMA_MAPPING_STATUS_UNKNOWN,
    IITPAVE_SCHEMA_SECTION_CONTEXT_BLOCK,
    IITPAVE_SCHEMA_SECTION_OUTPUT_HEADER,
    IITPAVE_SCHEMA_SECTION_STRESS_STRAIN_CONTEXT,
    IITPAVE_SCHEMA_SECTION_STRESS_STRAIN_TABLE_CANDIDATE,
    IITPAVE_SCHEMA_MANIFEST_STATUS_AUDIT_READY,
    IITPAVE_SCHEMA_MANIFEST_STATUS_BLOCKED_UNKNOWN_SCHEMA,
    IITPAVE_SCHEMA_MANIFEST_STATUS_NO_FIXTURES,
    IITPAVE_SCHEMA_MANIFEST_STATUS_NO_VERIFIED_FIXTURES,
)
from .rehab_engine import (
    CATEGORY_LABELS as REHAB_CATEGORY_LABELS,
    CATEGORY_NEXT_MODULE as REHAB_CATEGORY_NEXT_MODULE,
    CATEGORY_PRIORITY as REHAB_CATEGORY_PRIORITY,
    DEFAULT_RULES as REHAB_DEFAULT_RULES,
    DEFAULT_THRESHOLDS as REHAB_DEFAULT_THRESHOLDS,
    PLACEHOLDER_NOTE as REHAB_PLACEHOLDER_NOTE,
    REFERENCES as REHAB_REFERENCES,
    RecommendationContext,
    RehabSynthesisResult,
    RehabThresholds,
    TC_CRACK_SEALING,
    TC_MICRO_SURFACING,
    TC_OVERLAY,
    TC_POTHOLE_PATCHING,
    TC_RECONSTRUCTION,
    TC_ROUTINE_MAINTENANCE,
    TC_SLURRY_SEAL,
    TC_SURFACE_TREATMENT,
    TREATMENT_CATEGORIES,
    TreatmentRecommendation,
    compute_rehab_recommendations,
    get_thresholds as get_rehab_thresholds,
    reset_thresholds as reset_rehab_thresholds,
    set_thresholds as set_rehab_thresholds,
)

__all__ = [
    "linear_interpolate",
    "bracket_interpolate",
    "GradationInput",
    "GradationResult",
    "compute_gradation",
    "CoarseAggSGInput",
    "FineAggSGInput",
    "BitumenSGInput",
    "SpecificGravityResult",
    "compute_coarse_sg",
    "compute_fine_sg",
    "compute_bitumen_sg",
    "compute_bulk_sg_blend",
    "GmbSpecimen",
    "GmbGroup",
    "GmbInput",
    "GmbResult",
    "compute_gmb",
    "GmmSampleRaw",
    "GmmInput",
    "GmmResult",
    "compute_gmm",
    "StabilitySpecimen",
    "StabilityFlowInput",
    "StabilityFlowResult",
    "compute_stability_flow",
    "MarshallSummary",
    "MarshallRow",
    "build_marshall_summary",
    "compute_obc",
    "properties_at_obc",
    "OBCResult",
    "spec_target_air_voids",
    "MixSpec",
    "MixTypeRecord",
    "ComplianceResult",
    "check_compliance",
    "reload_specs",
    "save_specs",
    "MIX_SPECS",
    "MIX_TYPES",
    "MixDesignInput",
    "MixDesignResult",
    "compute_mix_design",
    "MaterialCalcInput",
    "MaterialBlock",
    "DryMaterialRow",
    "MaterialCalcResult",
    "compute_material_calc",
    "ImportedMixResult",
    "parse_summary_excel",
    "BinderGrade",
    "BINDER_GRADES",
    "PROPERTY_LABELS",
    "load_binders",
    "reload_binders",
    "StructuralInput",
    "PavementLayer",
    "StructuralResult",
    "compute_design_traffic",
    "compute_subgrade_mr",
    "suggest_composition",
    "compute_structural_design",
    "OverlayInput",
    "OverlayResult",
    "compute_overlay",
    "characteristic_deflection",
    "allowable_deflection",
    "temperature_corrected_deflection",
    "ColdMixInput",
    "ColdMixResult",
    "compute_cold_mix",
    "MicroSurfacingInput",
    "MicroSurfacingResult",
    "compute_micro_surfacing",
    "CodeRef",
    "CodeRecord",
    "CODE_REGISTRY",
    "get_code",
    "parse_refs",
    "code_labels",
    "APPLICATION_CONFIG_ENV_VAR",
    "CONFIG_SCHEMA_VERSION",
    "PROFILE_CONSULTANCY",
    "PROFILE_DEFAULT",
    "PROFILE_DEMO",
    "PROFILE_LAB",
    "SUPPORTED_PROFILE_KEYS",
    "ApplicationConfig",
    "ApplicationProfile",
    "ConfigMetadata",
    "ConfigValidationIssue",
    "ConfigValidationResult",
    "config_report_rows",
    "list_application_profiles",
    "load_application_config",
    "normalize_profile_key",
    "resolve_application_config",
    "validate_application_config",
    "validate_config_payload",
    "validate_profile_key",
    "DEPLOYMENT_CHECK_FAIL",
    "DEPLOYMENT_CHECK_PASS",
    "DEPLOYMENT_CHECK_WARN",
    "DEPLOYMENT_MANIFEST_FORMAT",
    "DEPLOYMENT_MANIFEST_VERSION",
    "DEPLOYMENT_SEVERITY_ERROR",
    "DEPLOYMENT_SEVERITY_INFO",
    "DEPLOYMENT_SEVERITY_WARNING",
    "LOCAL_INSTALLER_PREPARATION_FORMAT",
    "LOCAL_INSTALLER_PREPARATION_VERSION",
    "DeploymentChecklistItem",
    "DeploymentDiagnostics",
    "DeploymentIssue",
    "DeploymentManifest",
    "DeploymentPackagingChecklist",
    "LocalInstallerAssetCheck",
    "LocalInstallerPreparationChecklist",
    "RuntimePathCheck",
    "build_deployment_manifest",
    "build_deployment_packaging_checklist",
    "build_local_installer_preparation_checklist",
    "default_runtime_paths",
    "format_deployment_checklist_item_text",
    "format_deployment_packaging_checklist_text",
    "startup_deployment_diagnostics",
    "validate_runtime_environment",
    "validate_runtime_path",
    "write_deployment_manifest",
    "BENCHMARK_DATASET_READINESS_FORMAT",
    "BENCHMARK_DATASET_READINESS_VERSION",
    "BenchmarkDatasetCheck",
    "BenchmarkDatasetReadinessChecklist",
    "build_benchmark_dataset_readiness_checklist",
    "LayerInput",
    "LayerResult",
    "MaterialQuantityInput",
    "MaterialQuantityResult",
    "LAYER_TYPES",
    "compute_layer",
    "compute_material_quantity",
    "TrafficInput",
    "TrafficResult",
    "TERRAINS",
    "LANE_CONFIGS",
    "vdf_preset",
    "ldf_preset",
    "traffic_category",
    "compute_traffic_analysis",
    "DISTRESS_TYPES",
    "SEVERITY_LEVELS",
    "DistressType",
    "DistressRecord",
    "ConditionSurveyInput",
    "ConditionSurveyResult",
    "PerDistressBreakdown",
    "RehabRecommendation",
    "PCICalibration",
    "PCI_DEFAULT_CALIBRATION",
    "get_pci_calibration",
    "set_pci_calibration",
    "reset_pci_calibration",
    "compute_condition_survey",
    "condition_category",
    "recommend_rehab",
    "PCI_RECALIBRATE_ME",
    "PCI_PLACEHOLDER_NOTE",
    # Phase 12 — rehab synthesis engine
    "RehabSynthesisResult",
    "RecommendationContext",
    "RehabThresholds",
    "TreatmentRecommendation",
    "TREATMENT_CATEGORIES",
    "TC_CRACK_SEALING",
    "TC_MICRO_SURFACING",
    "TC_OVERLAY",
    "TC_POTHOLE_PATCHING",
    "TC_RECONSTRUCTION",
    "TC_ROUTINE_MAINTENANCE",
    "TC_SLURRY_SEAL",
    "TC_SURFACE_TREATMENT",
    "REHAB_CATEGORY_LABELS",
    "REHAB_CATEGORY_NEXT_MODULE",
    "REHAB_CATEGORY_PRIORITY",
    "REHAB_DEFAULT_RULES",
    "REHAB_DEFAULT_THRESHOLDS",
    "REHAB_PLACEHOLDER_NOTE",
    "REHAB_REFERENCES",
    "compute_rehab_recommendations",
    "get_rehab_thresholds",
    "reset_rehab_thresholds",
    "set_rehab_thresholds",
    # Phase 13 — IITPAVE integration layer
    "IITPavePavementLayer",
    "PavementStructure",
    "IITPaveLoadConfig",
    "IITPaveEvaluationPoint",
    "IITPavePointResult",
    "MechanisticResult",
    "IITPaveRunner",
    "IITPaveStubRunner",
    "IITPaveExternalExeRunner",
    "build_iitpave_input",
    "parse_iitpave_output",
    "default_evaluation_points",
    "iitpave_from_structural_layers",
    "is_known_stub_output",
    "IITPAVE_INPUT_FORMAT_VERSION",
    "IITPAVE_STUB_OUTPUT_VERSION",
    "IITPAVE_EXECUTABLE_ENV_VAR",
    "IITPAVE_RUNNER_EXTERNAL",
    "IITPAVE_RUNNER_STUB",
    "SUPPORTED_IITPAVE_RUNNER_MODES",
    "IITPAVE_DEFAULT_EXE_FILENAME_WIN",
    "IITPAVE_DEFAULT_EXE_FILENAME_POSIX",
    "IITPaveRunnerConfig",
    "IITPaveRunnerSelectionResult",
    "IITPaveEnvironmentIssue",
    "IITPaveEnvironmentValidationResult",
    "IITPaveExecutableCandidate",
    "IITPaveDryRunConfig",
    "IITPaveDryRunResult",
    "IITPaveFixtureIntakeResult",
    "IITPaveFixtureRecord",
    "IITPaveOutputContractResult",
    "IITPaveParserContractMarker",
    "IITPaveParserContractSection",
    "IITPaveParserContractHarnessResult",
    "IITPaveVerifiedParserContractResult",
    "IITPaveSchemaSectionMapping",
    "IITPaveVerifiedFixtureSchemaMappingResult",
    "IITPaveFixtureSchemaManifest",
    "IITPaveFixtureSchemaManifestEntry",
    "IITPaveSchemaFamilyCoverage",
    "DEFAULT_FIXTURE_MANIFEST",
    "DEFAULT_OUTPUT_SUFFIXES",
    "FIXTURE_STATUS_OPERATOR_PENDING",
    "FIXTURE_STATUS_REJECTED",
    "FIXTURE_STATUS_UNVERIFIED",
    "FIXTURE_STATUS_VERIFIED",
    "SUPPORTED_FIXTURE_STATUSES",
    "bundled_iitpave_dir",
    "bundled_iitpave_exe_candidates",
    "bundled_iitpave_exe_path",
    "build_iitpave_fixture_schema_manifest",
    "check_iitpave_dry_run_readiness",
    "discover_iitpave_executable",
    "iitpave_runner_config_from_mapping",
    "intake_iitpave_fixture_folder",
    "intake_iitpave_output_fixture",
    "inspect_iitpave_output_contract",
    "inspect_iitpave_verified_parser_contract",
    "map_iitpave_verified_fixture_schema",
    "run_iitpave_parser_contract_harness",
    "select_iitpave_runner",
    "validate_iitpave_environment",
    "IITPAVE_OUTPUT_FORMAT_REAL_PENDING",
    "IITPAVE_OUTPUT_FORMAT_STUB",
    "IITPAVE_OUTPUT_FORMAT_UNKNOWN",
    "IITPAVE_OUTPUT_STATUS_EMPTY",
    "IITPAVE_OUTPUT_STATUS_MISSING",
    "IITPAVE_OUTPUT_STATUS_REAL_CONTRACT_PENDING",
    "IITPAVE_OUTPUT_STATUS_STUB_CONTRACT",
    "IITPAVE_OUTPUT_STATUS_UNREADABLE",
    "IITPAVE_OUTPUT_STATUS_UNSUPPORTED",
    "IITPAVE_PARSER_CONTRACT_STATUS_BLOCKED",
    "IITPAVE_PARSER_CONTRACT_STATUS_INCOMPLETE",
    "IITPAVE_PARSER_CONTRACT_STATUS_PARTIAL",
    "IITPAVE_PARSER_CONTRACT_STATUS_SECTIONS_DETECTED",
    "IITPAVE_PARSER_CONTRACT_STATUS_UNSUPPORTED",
    "IITPAVE_SECTION_HEADER",
    "IITPAVE_SECTION_LABELED_BLOCK",
    "IITPAVE_SECTION_TABLE_LIKE",
    "IITPAVE_SCHEMA_FAMILY_DIRECT_STRESS_STRAIN_TABLE",
    "IITPAVE_SCHEMA_FAMILY_ELASTIC_LAYER_STRESS_STRAIN_TABLE",
    "IITPAVE_SCHEMA_FAMILY_UNKNOWN",
    "IITPAVE_SCHEMA_MAPPING_STATUS_BLOCKED",
    "IITPAVE_SCHEMA_MAPPING_STATUS_MAPPED",
    "IITPAVE_SCHEMA_MAPPING_STATUS_UNKNOWN",
    "IITPAVE_SCHEMA_SECTION_CONTEXT_BLOCK",
    "IITPAVE_SCHEMA_SECTION_OUTPUT_HEADER",
    "IITPAVE_SCHEMA_SECTION_STRESS_STRAIN_CONTEXT",
    "IITPAVE_SCHEMA_SECTION_STRESS_STRAIN_TABLE_CANDIDATE",
    "IITPAVE_SCHEMA_MANIFEST_STATUS_AUDIT_READY",
    "IITPAVE_SCHEMA_MANIFEST_STATUS_BLOCKED_UNKNOWN_SCHEMA",
    "IITPAVE_SCHEMA_MANIFEST_STATUS_NO_FIXTURES",
    "IITPAVE_SCHEMA_MANIFEST_STATUS_NO_VERIFIED_FIXTURES",
    "IITPAVE_REFERENCES",
    "IITPAVE_PLACEHOLDER_NOTE",
    "IITPAVE_SOURCE_STUB",
    "IITPAVE_SOURCE_EXTERNAL",
    # Phase 14 — mechanistic validation
    "MechanisticValidationInput",
    "MechanisticValidationSummary",
    "compute_mechanistic_validation",
    "FatigueCalibration",
    "FatigueCheck",
    "DEFAULT_FATIGUE_CALIBRATION",
    "compute_fatigue_life",
    "refused_fatigue_check",
    "RuttingCalibration",
    "RuttingCheck",
    "DEFAULT_RUTTING_CALIBRATION",
    "compute_rutting_life",
    "refused_rutting_check",
    "StrainExtraction",
    "extract_fatigue_strain",
    "extract_rutting_strain",
    "LABEL_FATIGUE",
    "LABEL_RUTTING",
    "get_fatigue_calibration",
    "set_fatigue_calibration",
    "reset_fatigue_calibration",
    "get_rutting_calibration",
    "set_rutting_calibration",
    "reset_rutting_calibration",
    "MECH_VALIDATION_PLACEHOLDER_NOTE",
    "MECH_VALIDATION_REFERENCES",
    "REFUSED_PLACEHOLDER_MECH",
    "REFUSED_MISSING_STRAIN",
    "REFUSED_MISSING_E_BC",
]
